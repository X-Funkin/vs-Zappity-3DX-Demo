...
export(NodePath) var sprite_mesh
...
func realign_sprite(): #Realigns the sprite to the set aligment
	if sprite_realignment:
		...
		var character_mesh : CSGMesh = get_node(sprite_mesh)
		var sprite_frame = character_node.frames.get_frame(character_node.animation,character_node.frame)
		character_mesh.material.set_shader_param("Sprite_Texture", sprite_frame)
		...
		character_mesh.translation.x = center_offset.x*pixel_size
		character_mesh.translation.y = center_offset.y*pixel_size
		character_mesh.scale.x = sprite_frame.get_size().x*pixel_size
		character_mesh.scale.y = sprite_frame.get_size().y*pixel_size
	pass
