...
func _process(delta):
	$"Right Note".position.y = get_parent().get_parent().song_cursor
#	$"Right Note".position.y = get_global_mouse_position().y
func mouse_entered():
	print("|ya eahy")

func _on_Player_Editor_Area_mouse_entered():
	print("ye aaahh")
	$"Right Note".visible = true


func _on_Player_Editor_Area_mouse_exited():
	print("AAE AOEOAE AEOOOA AA EOAAA EOOOEE EE")
	$"Right Note".visible = false
	...
	if event is InputEventMouseButton and event.is_pressed():
		...
		if event.button_index == BUTTON_LEFT:
			if Input.is_key_pressed(KEY_SHIFT):
				var notes = get_parent().get_right_notes()
				if notes != []:
