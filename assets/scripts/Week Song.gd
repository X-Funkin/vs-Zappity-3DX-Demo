...
func recieve_beat(beat_n):
	$"Camera/Screen Elements/beat".modulate = Color("00FFFF")
	if beatthing:
		$"Camera/Screen Elements/beat".modulate = Color("FF0000")
	beatthing = !beatthing
	pass

func save_song_data():
	var file = File.new()
	file.open(song_data_file, File.READ_WRITE)
	var song_data = JSON.parse(file.get_as_text()).result
#	print("read data ", song_data)
	if !song_data:
#		print("!song_data")
		song_data = {"songs":{}}
	if not "songs" in song_data:
#		print("not song in song_data")
		song_data["songs"] = {}
#		song_data = {"songs":{}}
	var save_data = {}
#	print("eyahd")
#	print(typeof(save_data))
#	print(typeof(song_data))
#	print(TYPE_NIL)
	var file_name = filename.split("/")[-1]
	save_data["song_path"] = filename
	...
	if not file_name in song_data["songs"]:
		for song in song_data["songs"]:
			if song_data["songs"][song]["song_path"] == filename:
				print("is it reasitnsg ", song_data["songs"][song], " ", filename)
				song_data["songs"].erase(song)
	song_data["songs"][file_name] = save_data
	print("save data", save_data)
	print(JSON.print(song_data))
	file.store_string(JSON.print(song_data))
	file.close()
#	var tex : Texture
#	tex.resource_path
#	if player_track:
